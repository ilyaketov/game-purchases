...
class Pipeline:
    ...
    def __init__(
        ...
        self.df1 = self._load(report1_path) if report1_path else pd.DataFrame()
        self.df2 = self._load(report2_path) if report2_path else pd.DataFrame()
        self.genba = self._load(genba_path) if genba_path else pd.DataFrame()

    @staticmethod
    def _load(path) -> pd.DataFrame:
        return pd.read_excel(path, dtype={
            # Можно потом тут зафиксировать типы для скорости
        })
    ...
    # -----------------------------------------------------------------------
    # Удобный шорткат для веб-приложения
    # -----------------------------------------------------------------------
