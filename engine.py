...
class Pipeline:
    ...
    def aggregate(self, ploshadka_key: str) -> pd.DataFrame:
        ...
        # Суммы prod_amount по валюте (для PLAION/KRM/Giftcard, где base = 0)
        for ccy in ("EUR", "USD", "TRY"):
            combined[f"prod_amount_{ccy.lower()}"] = combined.apply(
                lambda r, c=ccy: r["prod_amount"] if r.get("prod_ccy") == c else None,
                axis=1,
            )
        ...
        agg = (
            ...
            .agg(
                ...
                sum_prod_eur=("prod_amount_eur", "sum"),
                sum_prod_usd=("prod_amount_usd", "sum"),
                sum_prod_try=("prod_amount_try", "sum"),
    ...
    def _compute_price(row, genba_lookup):
        ...
        Порядок:
        1. CNY-поставщики (Kishmish/OneMoreTime/Callback) — RUB-сумма / qty / 11
        2. Genba — цена из genbaFile, fallback на USD-сумму
        3. TRY-поставщики (KRM) — TRY-сумма / qty / TRY_USD_RATE → USD
        4. Прочие — sum_base / qty (старый путь). Fallback на sum_prod_<target_ccy>,
           если sum_base пуст/нулевой (PLAION в EUR, Giftcard Pro в USD).
        """
        ...
        # 3. TRY-поставщики: TRY-сумма из prod_amount → USD
        if sg in TRY_SUPPLIERS:
            sum_try = row.get("sum_prod_try")
            if row["qty"] > 0 and pd.notna(sum_try) and sum_try != 0:
                return (sum_try / row["qty"] / TRY_USD_RATE, "USD")
            return (None, "USD")

        # 4. Прочие: основной путь — sum_base / qty
        if row["qty"] > 0 and pd.notna(row.get("sum_base")) and row["sum_base"] != 0:
            return (row["sum_base"] / row["qty"], target_ccy)

        # 4b. Fallback: sum_prod в целевой валюте (PLAION/Giftcard, где base = 0)
        sum_prod_key = f"sum_prod_{target_ccy.lower()}"
        sum_prod = row.get(sum_prod_key)
        if row["qty"] > 0 and pd.notna(sum_prod) and sum_prod != 0:
            return (sum_prod / row["qty"], target_ccy)
