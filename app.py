    ...
    for i, key in enumerate(selected):
        ...
        try:
            ...
            if agg.empty:
                ...
                continue
            xlsx_path = pipeline.save_to_excel(
                agg, key, out_dir / f"{key}_zakup_svod.xlsx"
            )
            csv_path = pipeline.save_to_csv(
                agg, key, out_dir / f"{key}_zakup_svod.csv"
            )
...
if "results" in st.session_state:
    ...
    if success:
        ...
        is_excel = fmt.startswith("Excel")
        path_key = "xlsx_path" if is_excel else "csv_path"
        mime = (
        ...
        with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zf:
            for k, v in success.items():
                zf.write(v[path_key], arcname=Path(v[path_key]).name)
        zip_buffer.seek(0)

        dl_cols = st.columns([1, 3])
        with dl_cols[0]:
            st.download_button(
                f"Скачать всё ({'XLSX' if is_excel else 'CSV'} ZIP)",
                data=zip_buffer,
                file_name=f"reports_{'xlsx' if is_excel else 'csv'}.zip",
                mime="application/zip",
                type="primary",
                use_container_width=True,
                key=f"dl_all_{is_excel}",
            )
        ...
        for i in range(0, len(items_l), files_per_row):
            ...
            for col, (k, v) in zip(row_cols, chunk):
                with col:
                    with open(v[path_key], "rb") as f:
                        st.download_button(
                            k,
                            data=f.read(),
                            file_name=Path(v[path_key]).name,
                            mime=mime,
                            key=f"dl_{k}_{is_excel}",
                            use_container_width=True,
                        )
